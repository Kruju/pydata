...
def plot_correlation_heatmaps(df):
    """Create both regular and squared correlation matrix heatmaps"""
    # Calculate correlation matrices
    corr_matrix = df.corr().round(2)
    corr_matrix_squared = (corr_matrix ** 2 * 100).round(1)  # Convert to percentage
    ...
    # Plot regular correlation heatmap
    ...
    # Plot squared correlation heatmap (variance explained)
    sns.heatmap(corr_matrix_squared, annot=True, vmin=0, vmax=100,
                cmap="YlOrRd", linewidths=0.5, mask=mask,
                annot_kws={"size": 8}, 
                fmt='.1f', ax=ax2)
    ax2.set_title("Variance Explained (%)", pad=20)
...
def main():
    ...
    plot_correlation_heatmaps(df_cleaned)  # Replace the original heatmap function
